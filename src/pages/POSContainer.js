  ...
  const closeDialog = () => setDialog(DIALOG_CLOSED);
    ...
    const fetchProductByBarcode = async (barcode) => {
        ...
        const productRes  = await customFetch(
            `/models/m_product?$select=M_Product_ID,Name,Value,C_UOM_ID&$filter=IsSold eq true and IsActive eq true and toupper(Value) eq '${safeBarcode.toUpperCase()}'&$top=1`
        );
