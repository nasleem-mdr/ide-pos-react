...
const DIALOG_CLOSED = { 
  ...
  const closeDialog = () => setDialog(DIALOG_CLOSED);
    ...
    const addToCart = async (product) => {
        ...
        // ProductType bisa berupa string 'S' atau object {id:'S',...} tergantung sumber data
        ...
        if (!isService) {
            qtyOnHand = await fetchQtyOnHand(product.M_Product_ID);
        ...
        if (existingIndex >= 0) {
            setCart(prev => prev.map((item, i) =>
                i === existingIndex
                    ? { ...item, QtyOrdered: targetQty, QtyOnHand: isService ? 0 : qtyOnHand } // Simpan 0 jika Jasa
                    : item
            ));
        } else {
            setCart(prev => [...prev, {
                ...
                uomOptions,
                selectedUOM,
                QtyOnHand:   isService ? 0 : qtyOnHand, // Simpan 0 jika Jasa
            }]);
        }
    };
